...
- `POST /api/auth/refresh` — refresh token → 새 access token + 새 refresh token (rotation)
- `POST /api/auth/logout` — refresh token 폐기
- DB에 SHA-256 해시로 저장, 원본은 클라이언트만 보유
- 14일 TTL, 매일 새벽 3시 만료/폐기 토큰 자동 정리
- **탈취 감지**: 이미 폐기된 토큰으로 rotate 시도 → 해당 사용자 전체 토큰 폐기
- 프론트엔드: 401 수신 시 자동 refresh 후 원래 요청 재시도

### 보안 주의

- **localStorage JWT는 XSS에 노출된다.** 악성 스크립트 주입 시 토큰 탈취 위험.
- 운영 전 **SameSite=Strict HttpOnly cookie** 방식 전환 권장.
  - 전환 시 변경 범위: JwtAuthenticationFilter(쿠키 읽기), OAuth2SuccessHandler(Set-Cookie), WebSocket(HandshakeInterceptor로 쿠키→세션), 프론트엔드(localStorage 제거, credentials:'include')
